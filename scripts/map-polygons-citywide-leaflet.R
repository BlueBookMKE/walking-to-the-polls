...
polling.place.coords <- all.wards |>
...
all.ward.labels <- all.ward.stats |>
  group_by(ward_no) |>
  filter(!is.na(units)) |>
  summarise(ward_label = paste0("Ward ", unique(ward_no), " votes at ", unique(polling_place_name), ".<br>",
...
leaflet.walk_time4cat <- polygons.walk_time4cat |>
  ...
  addPolygons(data = all.wards, fill = T, fillOpacity = 0,
